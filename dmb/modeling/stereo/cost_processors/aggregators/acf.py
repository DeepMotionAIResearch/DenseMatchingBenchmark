...
class Acf2DAggregator(nn.Module):
    """
    Args:
        max_disp (int): max disparity
        in_planes (int): the channels of raw cost volume
    ...
    Inputs:
        raw_cost (Tensor): raw cost volume,
                in [BatchSize, MaxDisparity//4, Height//4, Width//4] layout
    ...
    def __init__(self, max_disp, in_planes=48, batch_norm=True):
        super(Acf2DAggregator, self).__init__()
        self.max_disp = max_disp
        self.in_planes = in_planes
        self.batch_norm = batch_norm
        ...
        planes = 64
        self.dres0 = nn.Sequential(
            conv_bn_relu(batch_norm, self.in_planes, planes, 3, 1, 1),
            conv_bn_relu(batch_norm, planes, planes, 3, 1, 1),
        )
        self.dres1 = nn.Sequential(
            conv_bn_relu(batch_norm, planes, planes, 3, 1, 1),
            conv_bn(batch_norm, planes, planes, 3, 1, 1)
        )
        self.dres2 = Hourglass2D(in_planes=planes, batch_norm=batch_norm)
        self.dres3 = Hourglass2D(in_planes=planes, batch_norm=batch_norm)
        self.dres4 = Hourglass2D(in_planes=planes, batch_norm=batch_norm)

        self.classif1 = nn.Sequential(
            conv_bn_relu(batch_norm, planes, planes, 3, 1, 1),
            nn.Conv2d(planes, max_disp, kernel_size=3, stride=1, padding=1, bias=False),
        )
        self.classif2 = nn.Sequential(
            conv_bn_relu(batch_norm, planes, planes, 3, 1, 1),
            nn.Conv2d(planes, max_disp, kernel_size=3, stride=1, padding=1, bias=False),
        ...
        self.deconv1 = nn.ConvTranspose2d(192, 192, 8, 4, 2, bias=False)
        self.deconv2 = nn.ConvTranspose2d(192, 192, 8, 4, 2, bias=False)
        self.deconv3 = nn.ConvTranspose2d(192, 192, 8, 4, 2, bias=False)

    def forward(self, raw_cost):
        B, C, H, W = raw_cost.shape
        # concat_fms: (BatchSize, Channels*2, MaxDisparity/4, Height/4, Width/4)
        cost0 = self.dres0(raw_cost)
        cost0 = self.dres1(cost0) + cost0
        ...
        cost1 = self.deconv1(cost1, [full_h, full_w])
        cost2 = self.deconv2(cost2, [full_h, full_w])
        cost3 = self.deconv3(cost3, [full_h, full_w])
