...
class DeepPrunerBackbone(nn.Module):
    """
    Backbone proposed in DeepPruner.
    Args:
    ...
    def __init__(self, in_planes=3, batch_norm=True):
        super(DeepPrunerBackbone, self).__init__()
        ...
        self.layer1 = self._make_layer(batch_norm, BasicBlock, 32, 1, 1, 1, 1)
        self.layer2 = self._make_layer(batch_norm, BasicBlock, 64, 2, 2, 1, 1)
        self.layer3 = self._make_layer(batch_norm, BasicBlock, 128, 1, 1, 1, 1)
        self.layer4 = self._make_layer(batch_norm, BasicBlock, 128, 1, 1, 2, 2)
