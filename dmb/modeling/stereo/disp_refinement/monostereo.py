...
from .utils.min_warp_error import WarpErrorRefinement

class MonoStereoRefinement(nn.Module):
    """
    The disparity refinement module proposed in MonoStereo.
    ...
    def forward(self, disps, left, right, leftImage, rightImage):
        # only one estimated disparity map in MonoStereo
        init_disp = disps[-1]
        ...
        for block in self.refine_blocks:
            refine_disps.append(block(refine_disps[-1], leftImage, rightImage))
